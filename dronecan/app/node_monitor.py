...
class NodeMonitor(object):
    TIMEOUT = dronecan.protocol.NodeStatus().OFFLINE_TIMEOUT_MS / 1000  # @UndefinedVariable
    ...
    def __init__(self, node):
        ...
        self._handle = node.add_handler(dronecan.protocol.NodeStatus, self._on_node_status)  # @UndefinedVariable
    ...
    def _on_node_status(self, e):
        ...
        if not entry.discovered and should_retry_now and not e.node.is_anonymous:
            ...
            e.node.request(dronecan.protocol.GetNodeInfo.Request(), node_id,  # @UndefinedVariable
    ...
    def _on_info_response(self, e):
        ...
        msg = (
            "[#{0:03d}:dronecan.protocol.GetNodeInfo] " +
